...
const API = (() => {
  ...
  async function getClients(includeArchived = false) {
    let query = supabase
      .from('clients')
      .select('id, client_name, project_code, category, status, created_at, am_id, overview, account_manager:employees!am_id(name), client_entities(id, entity_name), client_platforms(id, platform_name)')
      .order('client_name')
  ...
  async function getTimesheetEntries(employeeId, from, to) {
    return supabase
      .from('timesheets')
      .select('*, clients(client_name, project_code), entity:client_entities(entity_name)')
  ...
  async function getTeamTimesheetEntries(from, to, empId = null) {
    ...
    let q = supabase
      .from('timesheets')
      .select('*, employees(id, name, profile_image_url, department), clients(client_name, project_code), entity:client_entities(entity_name)')
      .gte('date', from)
      .lte('date', to)
