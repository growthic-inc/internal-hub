...
const ClientDirectory = (() => {
  ...
  async function init(user) {
    ...
    _pcPerms = {
      can_view:   true, // visible to all authenticated users
    ...
    if (pcCtrl) {
      ...
      if (_pcPerms?.can_manage) {
        ...
        if (addBtn) {
          ...
          addBtn.onclick = () => _openInternalProjectModal(null)
        }
      }
    }
    _loadProjectCodes()
  ...
  function _renderProjectCodesTable() {
    ...
    el.querySelectorAll('.pc-toggle-internal').forEach(btn =>
  ...
  function _openInternalProjectModal(project) {
    ...
    const renderEntList = () => {
      ...
      wrap.innerHTML = _ents.map((e, i) => `
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;">
          ...
          <button class="btn-icon-sm pc-ent-del" data-idx="${i}" type="button" title="Remove"
            style="flex-shrink:0;">
    ...
    document.getElementById('pc-ip-save')?.addEventListener('click', async () => {
      ...
      const { error } = isEdit
        ? await API.updateInternalProject(project.id, { name, project_code: code, description: desc, entities: ents })
        : await API.createInternalProject({ name, project_code: code, description: desc, entities: ents })
