/* ============================================================
   PEOPLE / ONBOARDING — HRMS admin console
   Relocated from /home — editing an employee, deactivating/
   reactivating, KYC documents, bank details (audit-logged),
   and adding new hires. Read-only directory browsing and the
   org chart stay in Growthic One for everyone, HR included.
   ============================================================ */
...
const People = (() => {
      ...
      <div class="page-inner">
        <div class="page-toolbar">
          <div class="tabs" id="ppl-tabs">
            ...
            <button class="tab-btn" data-tab="invite">Add Employee</button>
          </div>
        ...
        <tbody>
          ${list.map(e => `
            <tr class="row-clickable" data-id="${e.id}" style="cursor:pointer;">
              ...
              <td>
                ${e.profile_completed === false
                  ? '<span class="badge badge--warning" style="font-size:10px;">Incomplete</span>'
  ...
  /* ══════════════════════════════════════════════════════════
     EMPLOYEE PROFILE MODAL (admin — KYC, bank details, edit)
  ══════════════════════════════════════════════════════════ */
    ...
    Utils.openModal(`
      <div class="modal-body" style="padding:0;">
        ...
        <div style="padding:16px 24px;border-bottom:1px solid var(--border);">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
            <div class="people-field">
              <div class="people-field-label">Account Number</div>
              <div class="people-field-value">${emp.bank_account_number ? Utils.escapeHtml(emp.bank_account_number) : '—'}</div>
            </div>
            <div class="people-field">
              <div class="people-field-label">IFSC</div>
              <div class="people-field-value">${emp.bank_ifsc ? Utils.escapeHtml(emp.bank_ifsc) : '—'}</div>
            </div>
          </div>
        </div>
  ...
  function _openEditModal(empId) {
    ...
    Utils.openModal(`
      ...
      <div class="modal-body" style="max-height:70vh;overflow-y:auto;">
        ...
        <div style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border);">
          <div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.5px;
            color:var(--text-muted);margin-bottom:4px;">Bank Details</div>
          <div style="font-size:11px;color:var(--text-muted);margin-bottom:12px;">Every change here is logged — who, when, and what changed.</div>
          <div class="form-row">
            <div class="form-group">
              <label class="form-label">Account Number</label>
              <input class="form-input" id="ppl-edit-bank-account"
                value="${Utils.escapeHtml(emp.bank_account_number || '')}">
            </div>
            <div class="form-group">
              <label class="form-label">IFSC</label>
              <input class="form-input" id="ppl-edit-bank-ifsc"
                value="${Utils.escapeHtml(emp.bank_ifsc || '')}">
            </div>
          </div>
        </div>
  ...
  async function _saveEditEmployee(empId) {
    ...
    const newBankAccount = document.getElementById('ppl-edit-bank-account')?.value.trim() || null
    const newBankIfsc     = document.getElementById('ppl-edit-bank-ifsc')?.value.trim()    || null

    // Diff bank fields against the currently-loaded record so we only log
    // what actually changed, not every save.
    const existingEmp = _employees.find(e => e.id === empId) || {}
    const bankAuditRows = []
    if ((existingEmp.bank_account_number || null) !== newBankAccount) {
      bankAuditRows.push({
        employee_id: empId, field_name: 'bank_account_number',
        original_value: existingEmp.bank_account_number || null, updated_value: newBankAccount,
        changed_by: _user.id,
      })
    }
    if ((existingEmp.bank_ifsc || null) !== newBankIfsc) {
      bankAuditRows.push({
        employee_id: empId, field_name: 'bank_ifsc',
        original_value: existingEmp.bank_ifsc || null, updated_value: newBankIfsc,
        changed_by: _user.id,
      })
    }
...
ModuleRegistry.register({
  ...
  access:    (user) => user.role === 'super_admin'
    || HRMSApp.hasAccess('people', 'manage_employees', 'view_only'),
  features:  {
