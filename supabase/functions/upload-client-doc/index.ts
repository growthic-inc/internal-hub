...
// Uploads a Brand Guidelines or Service Agreement file to Google Drive.
//
// Drive path:
//   Client Documents / {clientName} / Brand Guidelines  / {filename}
//   Client Documents / {clientName} / Service Agreements / {filename}
//
// Files are set "viewer: anyone with link" so the webViewLink works
// without a Google login (same pattern as upload-receipt).
//
// Returns: { driveUrl: string }  — store this in clients.brand_guidelines_url
//                                   or clients.service_agreement_url
//
// Secrets required (same set as upload-to-drive / upload-receipt):
//   SUPABASE_URL
//   SUPABASE_ANON_KEY
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Make viewable by anyone with the link (so <a href> works without Google login)
...
/* ── Google Drive helpers (shared pattern) ───────────────── */
...
async function uploadFileToDrive(
  ...
  // Build a clean reader-accessible URL.
  // Drive returns /edit?rtpof=true for Office files (DOCX/DOC) which blocks
  // users who only have "reader" permission. Replace with the correct /view
  // path for each Google app, or the plain Drive /view for native files.
  const id  = uploadData.id
  const raw = uploadData.webViewLink || ''
  let viewUrl: string
  if (raw.includes('docs.google.com/document/d/')) {
    viewUrl = `https://docs.google.com/document/d/${id}/view`
  } else if (raw.includes('docs.google.com/spreadsheets/d/')) {
    viewUrl = `https://docs.google.com/spreadsheets/d/${id}/view`
  } else if (raw.includes('docs.google.com/presentation/d/')) {
    viewUrl = `https://docs.google.com/presentation/d/${id}/view`
  } else {
    // PDFs, images, and other native Drive files
