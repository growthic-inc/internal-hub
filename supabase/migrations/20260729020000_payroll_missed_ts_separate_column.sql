-- Add missed_timesheet_days as a separate column on payroll_records.
-- Previously missed TS days were lumped into unpaid_leave_days.
-- The JS deduction modal already references rec.missed_timesheet_days
-- so adding this column makes the UI breakdown work immediately.

ALTER TABLE payroll_records
  ADD COLUMN IF NOT EXISTS missed_timesheet_days INT NOT NULL DEFAULT 0;

-- Updated generate_monthly_payroll:
-- - Components stored as annual → divide by 12 for monthly salary
-- - unpaid_leave_days = approved deductible leave only
-- - missed_timesheet_days = working days with no submitted/approved timesheet
-- - deductions = per-day rate × (unpaid_leave_days + missed_timesheet_days)
...
BEGIN
  ...
  LOOP
    ...
    -- Missed timesheet days: working days (Mon–Sat, not holiday, not on any
    -- approved full-day leave) with no submitted/approved timesheet entry
    SELECT COUNT(*)::INT
    INTO v_missed_ts_days
    FROM generate_series(v_period_start, LEAST(v_lwd, v_period_end), '1 day'::interval) d(day)
    WHERE
      EXTRACT(DOW FROM d.day) != 0
      AND NOT EXISTS (
